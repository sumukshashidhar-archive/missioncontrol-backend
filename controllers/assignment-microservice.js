...
module.exports = {
  ...
  getAssignments: async (student_class, student_section) => {
      return new Promise(async (resolve, reject) => {
        assignment.find({class_assigned: student_class, section: student_section, open: true}, {_id:1, assignmentName: 1, assignmentLink: 1, dueDate: 1}, async(err, obj) => {
            if(err) {
                console.log(err)
                resolve(false)
            }
      ...
      return new Promise(async(resolve, reject) => {
        ...
        student.findOne({emailID: student_id}, {totalInteractionPoints: 1}, async(err, obj)=> {
            if(err) {
                ...
                if((duration == 1 && obj["totalInteractionPoints"] >= 20) || (duration == 2 && obj["totalInteractionPoints"] >= 50)) {
                    ...
                    assignment.findById({id:assingment_id}, async (err2, obj2) => {
                        if(err2) {
                            ...
                            for(let i=0; i<obj2["extensionPurchasedBy"].length; i++) {
                                if(obj2["extensionPurchasedBy"] == student_id) {
                                    ...
                                    student.updateOne({emailID: student_id}, {totalInteractionPoints: interactpoints}, async(err3, obj3)=>{
                                        if(err3) {
                                            resolve(false)
                                        }
                                        else {
                                            ...
                                            obj2["extensionPurchasedByNames"].append(obj3["student_name"])
                                            obj2["newDueDate"].append(newDue)

                                            // finally, assignment updating

                                            assingment.updateOne({_id: assingment_id}, {extensionPurchasedBy: obj2["extensionPurchasedBy"], extensionPurchasedByNames: obj2["extensionPurchasedByNames"], newDueDate: newDue}, async (err4, obj4) => {
                                                if(err4) {
                                                    resolve(false)
                                                }
                                                else {
